...
static void compute_syndromes(uint32_t *ecc, unsigned int *syn)
{
	...
	for (j = 0; j < t; j++) {
		synVal = syn[j];
		powVal = 2* a_log_tab[synVal];
		mFlag = (int)(((0 - synVal) & mask) >> 31); // if synVal > 0
		syn[2*j+1] = a_pow_tab[powVal - n * (powVal / n)] * mFlag;
	}
...
{
	unsigned int i, j, l, d, it, read_write_val;
	unsigned int tmp = 0, pd = 1, evalVal = 0, mask = 0x80000000, synVal = 0,elpVal = 0, elpDeg = 0, pelpVal = 0, pelpDeg = 0;
	...
	for (i = 0; (i < t); i++) {
		...
		tmp = (a_log_tab[d] + n - a_log_tab[pd]) * dFlag + tmp * !dFlag ;
		...
		for (j = 0; j <= t; j++) {
			...
			l = a_log_tab[pelpVal] * v;
			
			modu = (tmp+l) - n * ((tmp+l) / n);
			
			read_write_val = elp_c[a+k] ^ (a_pow_tab[modu] * v);
			elp_c[a+k] = read_write_val;
		}
...
static int chien_search(unsigned int len, unsigned int *p, unsigned int *roots, int d)
{
	...
	unsigned int log_pd_val = a_log_tab[pd];
	int logVal = a_log_tab[p0] + n - log_pd_val;

	x = (int)(((logVal - n) & mask) >> 31); // x = 1 if val < n
	v = (int)(((0 - p0) & mask) >> 31); // v = 1 if p0 > 0

	syn0 = a_pow_tab[logVal - (n*!x)] * v;
