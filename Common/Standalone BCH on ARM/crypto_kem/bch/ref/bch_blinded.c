...
uint32_t a_log_tab_blind_access_parallel7x(uint32_t index)
{
                ...
                for(j=0; j<510; j=j+3)
                {
                                ...
                                temp1 = (temp2&one7x) ^ one7x;            // 9-bit chunks of temp1 = [...1] onlly if the slot contains a 0
                                temp1 = temp1 * nine_ones;
 

                                //pack_7x_entries = (a_pow_tab[j+6]<<(6*9)) | (a_pow_tab[j+5]<<(5*9)) | (a_pow_tab[j+4]<<(4*9)) | (a_pow_tab[j+3]<<(3*9));
                                //pack_7x_entries = pack_7x_entries | (a_pow_tab[j+2]<<(2*9)) | (a_pow_tab[j+1]<<(1*9)) | (a_pow_tab[j]);
                                //printf("%lu, ", pack_7x_entries);
 

                                sum = sum + (temp1 & a_log_tab_packed[j/3]); // temp1 is 0 in all cases except j=index
                                j_packed = j_packed + seven7x;                // increment j by 7 in all positions            
               }
               
                temp1 = 510 ^ index;                                     // for j=511
                // check if any of the 9 bits is non-zero; temp2 will be either 0 or 1
                temp2 = (temp1>>8)|(temp1>>7)|(temp1>>6)|(temp1>>5)|(temp1>>4)|(temp1>>3)|(temp1>>2)|(temp1>>1)|(temp1);
               
                temp1 = (temp2&one) - one;      // temp1 = {0} if temp2=1 otherwise temp1 = {1}
                sum = sum + (temp1 & a_log_tab_packed[170]);               // temp1 is 0 in all cases except j=index
 

 

                temp1 = 511 ^ index;                                     // for j=511
                // check if any of the 9 bits is non-zero; temp2 will be either 0 or 1
                temp2 = (temp1>>8)|(temp1>>7)|(temp1>>6)|(temp1>>5)|(temp1>>4)|(temp1>>3)|(temp1>>2)|(temp1>>1)|(temp1);
               
                temp1 = (temp2&one) - one;      // temp1 = {0} if temp2=1 otherwise temp1 = {1}
                sum = sum + (temp1 & a_log_tab_packed[171]);               // temp1 is 0 in all cases except j=index
                //temp1 = (temp2&two) - two;  // temp1 = {0} if temp2=1 otherwise temp1 = {1}
                //sum = sum + (temp1 & a_log_tab_packed[170]);           // temp1 is 0 in all cases except j=index
                ...
                //temp1 = (sum & 134217727) | (sum>>27);
                //temp1 = (temp1 & 262143) | (temp1>>18);
                //temp1 = (temp1 & 511) | (temp1>>9);
...
uint32_t a_pow_tab_blind_access_parallel7x(uint32_t index)
{
                ...
                for(j=0; j<510; j=j+3)
                {
                                ...
                                sum = sum + (temp1 & a_pow_tab_packed[j/3]);              // temp1 is 0 in all cases except j=index
                                j_packed = j_packed + seven7x;                // increment j by 7 in all positions            
               }
                ...
                temp1 = (temp2&one) - one;      // temp1 = {0} if temp2=1 otherwise temp1 = {1}
                sum = sum + (temp1 & a_pow_tab_packed[170]);            // temp1 is 0 in all cases except j=index
                ...
                temp1 = (temp2&one) - one;      // temp1 = {0} if temp2=1 otherwise temp1 = {1}
                sum = sum + (temp1 & a_pow_tab_packed[171]);            // temp1 is 0 in all cases except j=index
...
static int compute_error_locator_polynomial(unsigned int *syn2,unsigned int * elp_c, unsigned int *deg)
{
	...
	for (i = 0; (i < t); i++) {
		...
		synVal = syn2[2*i+2];//elp_read(syn2,2*t,2*(i*a)+2);
		d = synVal * a + d * !a;
		elpDeg = elpDeg + 1;
		for (j = 1; j <= t; j++) {
			x = (int)(((j - (elpDeg)) & mask) >> 31) * a; // x = 1 if j <= elp->deg
			synVal = elp_read(syn2,2*t,2*i+2-(j*x));
			elpVal = elp_c[j];//elp_read(elp_c,ELP_SIZE,j*x);
