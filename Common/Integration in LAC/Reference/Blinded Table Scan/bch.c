...
uint64_t a_log_tab_blind_access_parallel7x(uint64_t index)
{
	...
	for(j=0; j<511; j=j+7)
	{
		...
		sum = sum + (temp1 & a_log_tab_packed[j/7]);	// temp1 is 0 in all cases except j=index
		j_packed = j_packed + seven7x;		// increment j by 7 in all positions	
 	}
	
	
	temp1 = nine_ones ^ index;			// for j=511
	// check if any of the 9 bits is non-zero; temp2 will be either 0 or 1
	temp2 = (temp1>>8)|(temp1>>7)|(temp1>>6)|(temp1>>5)|(temp1>>4)|(temp1>>3)|(temp1>>2)|(temp1>>1)|(temp1);
	temp1 = (temp2&one) - one;	// temp1 = {0} if temp2=1 otherwise temp1 = {1}
	sum = sum + (temp1 & a_log_tab_packed[73]);	// temp1 is 0 in all cases except j=index
...
uint64_t a_pow_tab_blind_access_parallel7x(uint64_t index)
{
	...
	for(j=0; j<511; j=j+7)
	{
		...
		sum = sum + (temp1 & a_pow_tab_packed[j/7]);	// temp1 is 0 in all cases except j=index
		j_packed = j_packed + seven7x;		// increment j by 7 in all positions	
 	}
	...
	temp2 = (temp1>>8)|(temp1>>7)|(temp1>>6)|(temp1>>5)|(temp1>>4)|(temp1>>3)|(temp1>>2)|(temp1>>1)|(temp1);
	temp1 = (temp2&one) - one;	// temp1 = {0} if temp2=1 otherwise temp1 = {1}
	sum = sum + (temp1 & a_pow_tab_packed[73]);	// temp1 is 0 in all cases except j=index
...
static int compute_error_locator_polynomial(unsigned int *syn2,unsigned int * elp_c, unsigned int *deg)
{
	...
	for (i = 0; (i < t); i++) {
		...
		for (j = 0; j <= t; j++) {
			...
			a = j;
			pelpVal = pelp_c[a];//elp_read(pelp_c,PELP_SIZE,a);
		...
		synVal = syn2[2*i+2]; //elp_read(syn2,2*t,2*i+2);
		d = synVal * a + d * !a;
		elpDeg = elpDeg + 1;
		for (j = 1; j <= t; j++) {
			x = (int)(((j - (elpDeg)) & mask) >> 31) * a; // x = 1 if j <= elp->deg
			synVal = elp_read(syn2,2*t,2*i+2-j);
			elpVal = elp_c[j];//elp_read(elp_c,ELP_SIZE,j);
